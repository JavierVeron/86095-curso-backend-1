...
const operacion = async () => {
    try {
        ...
        const stats = await fs.promises.stat(nombreArchivo);
        const info = {
        ...
        const resultado = await fs.promises.writeFile("info.json", contenido);

        if (resultado) {
            throw new Error("Error! No se pudo crear el archivo!");
        }
    } catch (error) {
        console.log(error);
    }
